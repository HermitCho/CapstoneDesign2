...
4) PlayerPrefabs으로 설정을 저장하며 토글 속성 있다. ex)IsMusicOn 배경음, IsSoundOn 효과음";
    ...
    /// <summary>
    /// 직접 AudioClip을 재생하는 함수 (플레이리스트에 없어도 가능)
    /// </summary>
    /// <param name="clip">재생할 AudioClip</param>
    /// <param name="volume">볼륨 (0-1)</param>
    /// <param name="pitch">피치</param>
    /// <param name="callback">재생 완료 후 콜백</param>
    /// <returns>AudioSource</returns>
    public AudioSource PlayDirectClip(AudioClip clip, float volume = 1f, float pitch = 1f, Action callback = null)
    {
        if (clip == null)
        {
            return null;
        ...
        return PlayOneShot(clip, Vector2.zero, volume, pitch, callback);
    }

    /// <summary>
    /// 3D 위치에서 직접 AudioClip을 재생하는 함수
    /// </summary>
    /// <param name="clip">재생할 AudioClip</param>
    ...
    /// <param name="callback">재생 완료 후 콜백</param>
    /// <returns>AudioSource</returns>
    public AudioSource PlayDirectClipAtPoint(AudioClip clip, Vector3 position, float volume = 1f, float pitch = 1f, Action callback = null)
    {
        if (clip == null)
        {
            return null;
        ...
        return PlayClipAtPoint(clip, position, volume, pitch, callback);
    }
    ...
    public void PlayBGMWithLoop(string clipName)
    {
        ...
        if (currentLoopingBGM == clipName && bgmLoopCoroutine != null)
        {
            return;
        }
